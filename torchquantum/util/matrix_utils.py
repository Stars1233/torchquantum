...
def ultra_precise_unitary(matrix, iterations=5, tolerance=1e-10):
    ...
    input_deviation = np.max(
        np.abs(np.conjugate(matrix.T) @ matrix - np.eye(matrix.shape[0]))
    )
    ...
    for i in range(iterations):
        ...
        try:
            ...
            H_sqrt_inv = (
                eigenvecs @ np.diag(1.0 / np.sqrt(eigenvals)) @ eigenvecs.conj().T
            )
            U_refined = U @ H_sqrt_inv

            new_deviation = np.max(
                np.abs(U_refined.conj().T @ U_refined - np.eye(U.shape[0]))
            )
        ...
        try:
            ...
            new_deviation = np.max(
                np.abs(U_norm.conj().T @ U_norm - np.eye(U.shape[0]))
            )
        ...
        if i == iterations - 1 and best_deviation > tolerance:
            try:
                ...
                V_dp, _, Wh_dp = scipy.linalg.svd(
                    matrix_dp, full_matrices=True, lapack_driver="gesdd"
                )
                U_dp = V_dp @ Wh_dp

                new_deviation = np.max(
                    np.abs(U_dp.conj().T @ U_dp - np.eye(U_dp.shape[0]))
                )
    ...
    if best_deviation > tolerance:
        ...
        try:
            V, _, Wh = scipy.linalg.svd(
                matrix, full_matrices=True, lapack_driver="gesdd"
            )
            U_final = V @ Wh
            final_deviation = np.max(
                np.abs(U_final.conj().T @ U_final - np.eye(U_final.shape[0]))
            )
    ...
    final_deviation = np.max(
        np.abs(final_matrix.conj().T @ final_matrix - np.eye(final_matrix.shape[0]))
    )
