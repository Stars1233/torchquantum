...
def gradient_circuit(left, target, right, n_wires, observable):
    ...
    for coeff, pauli in zip(target["coeffs"], target["paulis"]):
        ...
        dev = tq.QuantumDevice(
            n_wires=n_wires + 1, bsz=1, device="cpu"
        ...
        for op_info in left:
            if "coeffs" in op_info:
                # Evolution
                op = tq.operator.OpPauliExp(
                    coeffs=op_info["coeffs"],
                    paulis=op_info["paulis"],
                    theta=op_info["params"],
                    trainable=False,
                )
        ...
        dev.controlled_unitary(
            params=generator.matrix, c_wires=[ancilla_qubit], t_wires=target["wires"]
        )
        # ancilla
        dev.h(wires=ancilla_qubit)
        # right
